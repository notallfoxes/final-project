...
We will now prepare to fit the decision tree model in a similar fashion by setting our engine to `rpart`.
...
Looks like the decision tree 

```{r class.source = 'fold-show'}
result_decision <- augment(tree_fit, new_data = w_train) %>%
...
In contrast, it performed much better with the standard and the cartoon styles, incorrectly predicting only 17.1% and 31.5% of the time, respectively.
```{r}
...
tree_rs <- tune_grid(
  tree_spec,
  style ~ rating + views  + daily_pass + free_episodes  + likes + subscribers + synopsis_length + monday + tuesday + wednesday + thursday + friday + saturday + sunday + ongoing + completed + hiatus + english + korean + indonesian + action + comedy + drama + fantasy + heartwarming + historical + horror + mystery + romance + scifi + slice_of_life + sports + super_hero + supernatural + thriller,
  resamples = w_folds,
  grid = tree_grid,
