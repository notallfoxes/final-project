...
![(from left) Wished You Were Dead, Ghost Teller](images/standard.png){width="250"}
...
![(from left) Lore Olympus, Bluechair, School Bus Graveyard, Supersonic Girl](images/cartoon.png){width="720"}
...
![(from left) Annarasumanara, Covenant](images/realistic_standard.png){width="250"}
...
![(from left) Secret Playlist, True Beauty](images/korean_realism.png){width="120"}
</center>
...
result_dt
```
The multinomial model still reigns. Out of curiosity, let's see what the model predicted.
...
result_knn
```
The nearest neighbors model still can't beat the multinomial one. Let's examine the heatmap to see if there are any notable differences.
...
augment(knn_fit, new_data = w_test) %>%
  ...
  scale_y_discrete(limits = rev)
```
We see a similar problem. KNN can't decipher what makes a style Korean realism, but we do see one failed guess at a realistic standard style comic this time!
...
conf_mat(predict_best, truth = style, estimate = .pred_class) %>%   autoplot(type = "heatmap") +
  ...
  scale_y_discrete(limits = rev)
```
Again, it looks like our model can't distinguish the Korean realism style at all. I find it interesting that the only times it even tried to predict it, it incorrectly guessed the standard style. 
